...
const createCustomer = async (req, res) => {
  ...
  const full_name = reverseHebrow(encode(req.body.full_name).trim());
  ...
  const street = reverseHebrow(encode(req.body.street).trim());
  const city = reverseHebrow(encode(req.body.city).trim());
...
const reverseHebrow = (str) => {
  if (!validator.isAlphanumeric(str.replace(" ", ""), "he"))
